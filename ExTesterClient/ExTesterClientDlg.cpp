...
// ExTesterClientDlg.cpp: 구현 파일
//

#include "pch.h"
#include "framework.h"
#include "ExTesterClient.h"
#include "ExTesterClientDlg.h"
...
{
	auto ptrCExTesterClientDlg = (CExTesterClientDlg*)param;
	CEdit* ptrEditLog = &ptrCExTesterClientDlg->m_editLog;
	...
	if (wasErrorCode != 0) {
		...
		int len = ptrCExTesterClientDlg->m_editLog.GetWindowTextLength();
		ptrEditLog->SetSel(len, len);
		ptrEditLog->ReplaceSel(str);
		return 0;
	}

	g_hClient = socket(PF_INET, SOCK_STREAM, IPPROTO_HOPOPTS);

	SOCKADDR_IN tAddr = {};
	tAddr.sin_family = AF_INET;
	tAddr.sin_port = htons(g_serverPort);
	tAddr.sin_addr.s_addr = inet_addr(LPCTSTR(g_serverIp));


	connect(g_hClient, (struct sockaddr*)&tAddr, sizeof(tAddr));
	...
	while (g_serverRunningState)
	{
		...
		for (int i = 0; i < readSet.fd_count; ++i)
		{
			if (FD_ISSET(readSet.fd_array[i], &copySet))
			{
				...
				//CString recvData = CExTesterClientDlg::ConvertUtf8((byte*)packet, recvLength);
				if (packet[0] == 0x02) {
					packet[0] = '2';
				}
				...
				ptrCExTesterClientDlg->ShowRecvData(ptrCExTesterClientDlg, packet, recvLength);
			}
		}
	...
	CString str = "";
	str.Format("------ Stop Client ------\n");
	ptrCExTesterClientDlg->AddEditLog(str);

	ptrCExTesterClientDlg->GetDlgItem(IDC_BTN_CON)->EnableWindow(true);
	ptrCExTesterClientDlg->GetDlgItem(IDC_BTN_STOP)->EnableWindow(false);

	return 0;
}

void CExTesterClientDlg::ShowRecvData(CExTesterClientDlg* dlg, char* packet, int packetSize) {
...
CExTesterClientDlg::CExTesterClientDlg(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_EXTESTERCLIENT_DIALOG, pParent)
{
...
CExTesterClientDlg::~CExTesterClientDlg()
{
...
void CExTesterClientDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CExTesterClientDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BTN_CON, &CExTesterClientDlg::OnBnClickedBtnConnect)
	ON_BN_CLICKED(IDC_BTN_STOP, &CExTesterClientDlg::OnBnClickedBtnStop)
	ON_BN_CLICKED(IDC_BTN_SEND, &CExTesterClientDlg::OnBnClickedBtnSend)
	ON_BN_CLICKED(IDC_BTN_CREATE_PACKET, &CExTesterClientDlg::OnBnClickedBtnCreatePacket)
	ON_WM_SYSCOMMAND()
	ON_BN_CLICKED(IDC_BTN_RESET_EDIT, &CExTesterClientDlg::OnBnClickedBtnResetEdit)
	ON_BN_CLICKED(IDC_BTN_CLEAR_LOG, &CExTesterClientDlg::OnBnClickedBtnClearLog)
	ON_BN_CLICKED(IDC_BTN_CREATE_ITGR_PACKET, &CExTesterClientDlg::OnBnClickedBtnCreateItgrPacket)
	ON_BN_CLICKED(IDC_BTN_CREATE_POST_PACKET, &CExTesterClientDlg::OnBnClickedBtnCreatePostPacket)
	ON_BN_CLICKED(IDC_BTN_CREATE_CMP_PACKET, &CExTesterClientDlg::OnBnClickedBtnCreateCmpPacket)
	ON_BN_CLICKED(IDC_BTN_CREATE_HIPLUS_PACKET, &CExTesterClientDlg::OnBnClickedBtnCreateHiplusPacket)
END_MESSAGE_MAP()


// CExTesterClientDlg 메시지 처리기

BOOL CExTesterClientDlg::OnInitDialog()
{
	...
	SetDlgItemText(IDC_EDIT_IP, "192.168.1.206"); 
	//SetDlgItemText(IDC_EDIT_IP, "127.0.0.1");
	SetDlgItemInt(IDC_EDIT_PORT, 44444);
...
void CExTesterClientDlg::OnPaint()
{
	if (IsIconic())
	{
...
//  이 함수를 호출합니다.
HCURSOR CExTesterClientDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

BOOL CExTesterClientDlg::PreTranslateMessage(MSG* pMsg)
{
...
void CExTesterClientDlg::OnBnClickedBtnResetEdit()
{
...
void CExTesterClientDlg::OnBnClickedBtnConnect()
{
	int port = GetDlgItemInt(IDC_EDIT_PORT);
	g_serverPort = port;

	GetDlgItemText(IDC_EDIT_IP, g_serverIp);
	...
	if (!g_serverRunningState) {
		...
		CString str = "";
		str.Format("------ Start Client ------\n");
		AddEditLog(str);

		GetDlgItem(IDC_BTN_CON)->EnableWindow(false);
		GetDlgItem(IDC_BTN_STOP)->EnableWindow(true);
	}
}

void CExTesterClientDlg::OnBnClickedBtnStop()
{
	g_serverRunningState = false;
	...
	GetDlgItem(IDC_BTN_CON)->EnableWindow(true);
	GetDlgItem(IDC_BTN_STOP)->EnableWindow(false);
}

int CExTesterClientDlg::ConvertMultibyteToUtf8(CString multibyte_data, byte* utf8_data, int utf8_data_size)
{
...
CString CExTesterClientDlg::ConvertUtf8(byte* utf8_data, int utf8_data_size)
{
	wchar_t strUnicode[PACKET_LENGTH];
...
void CExTesterClientDlg::AddEditLog(CString log)
{
	CString str = log + "\n";
...
void CExTesterClientDlg::OnBnClickedBtnSend()
{
	if (!g_serverRunningState) {
		AddEditLog("--- Client is not connected....");
		return;
	}
...
void CExTesterClientDlg::OnBnClickedBtnCreatePacket()
{
	CTime cTime = CTime::GetCurrentTime();

	CString date;
	date.Format("%04d%02d%02d%02d%02d%02d",
		cTime.GetYear(),
		cTime.GetMonth(),
		cTime.GetDay(),
		cTime.GetHour(),
	...
	m_editChDate.SetWindowTextA(date);
...
void CExTesterClientDlg::OnBnClickedBtnClearLog()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
void CExTesterClientDlg::OnBnClickedBtnCreateItgrPacket()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CTime cTime = CTime::GetCurrentTime();
	...
	CString workDate = date + date;

	m_editChLength.SetWindowTextA("0060");
	m_editChTypeSend.SetWindowTextA("MC0L00");
	m_editChTypeRecv.SetWindowTextA("054999");
	m_editChWork.SetWindowTextA("0499");
	m_editChJob.SetWindowTextA("0010");
	m_editChResponse.SetWindowTextA("0000");
	m_editData.SetWindowTextA(workDate + "00000");
	OnBnClickedBtnCreatePacket();
}


void CExTesterClientDlg::OnBnClickedBtnCreatePostPacket()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CTime cTime = CTime::GetCurrentTime();
	...
	m_editChTypeSend.SetWindowTextA("MC0L00");
	m_editChTypeRecv.SetWindowTextA("054999");
	m_editChWork.SetWindowTextA("0484");
	m_editChJob.SetWindowTextA("0010");
	m_editChResponse.SetWindowTextA("0000");
	m_editData.SetWindowTextA(workDate + "00000");
	OnBnClickedBtnCreatePacket();
...
void CExTesterClientDlg::OnBnClickedBtnCreateCmpPacket()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CTime cTime = CTime::GetCurrentTime();
	...
	m_editChTypeSend.SetWindowTextA("MC0L00");
	m_editChTypeRecv.SetWindowTextA("054999");
	m_editChWork.SetWindowTextA("0802");
	m_editChJob.SetWindowTextA("0010");
	m_editChResponse.SetWindowTextA("0000");
	m_editData.SetWindowTextA(workDate + "00000");
	OnBnClickedBtnCreatePacket();
...
void CExTesterClientDlg::OnBnClickedBtnCreateHiplusPacket()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_editChLength.SetWindowTextA("0059");
	m_editChTypeSend.SetWindowTextA("MC0L00");
	m_editChTypeRecv.SetWindowTextA("054999");
	m_editChWork.SetWindowTextA("0051");
	m_editChJob.SetWindowTextA("0010");
	m_editChResponse.SetWindowTextA("0000");
	m_editData.SetWindowTextA("20210622202106220001");
	OnBnClickedBtnCreatePacket();
}
